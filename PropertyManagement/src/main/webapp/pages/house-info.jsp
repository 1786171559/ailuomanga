...
<script>
    layui.use('table', function () {
        ...
        table.render({
            ...
            , url: '${pageContext.request.contextPath}/house/getHouseInfo?v=' + new Date().getTime()//数据接口
            , cols: [[
                //表头
                {type: 'checkbox', fixed: 'left'}
                , {field: 'houseId', title: 'ID', width: 80, sort: true}
                , {field: 'houseNum', title: '门牌号', width: 80, sort: true}
                , {field: 'deptNum', title: '楼号', width: 80, sort: true}
                , {field: 'houseType', title: '类型', width: 95, sort: true}
                , {field: 'address', title: '地区', width: 80}
                , {field: 'sellStatus', title: '出售状态', width: 120, sort: true}
                , {field: 'direction', title: '朝向', width: 120}
                , {field: 'memo', title: '描述', width: 140,}
                , {field: 'userId', title: '用户id', width: 140, hide: true}
                , {
                    field: 'userName',
                    title: '用户名',
                    width: 90,
                    templet: '<div> {{# if(d.userInfo=== null){ }}无 {{# }else{ }}{{d.userInfo.userName}}{{# } }}</div>'
                }
                , {
                    field: 'phone',
                    title: '联系方式',
                    width: 140,
                    templet: '<div> {{# if(d.userInfo=== null){ }}无 {{# }else{ }}{{d.userInfo.phone}}{{# } }}</div>'
                }
                , {
                    field: 'carId',
                    title: '车牌号',
                    width: 120,
                    templet: '<div> {{# if(d.userInfo=== null){ }}无 {{# }else{ }}{{d.userInfo.carId}}{{# } }}</div>'
                }
                , {fixed: 'right', title: '操作', width: 160, toolbar: '#DoTool'}
            ]]
            , page: true //开启分页
        ...
        table.on('tool(demo)', function (obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('真的删除么？', function () {
                    $.ajax({
                        // 编写json格式，设置属性和值
                        url: "${pageContext.request.contextPath}/house/delHouseByHouseId",
                        data: {"houseId": data.houseId},
                        dataType: "json",
                        type: "post",
                        success: function (data) {
                            if (data.flag == "success") {
                                window.location.href = "${pageContext.request.contextPath}/house/house-info"
                            } else {
                ...
                parent.layer.open({
                    ...
                    maxmin: true,
                    closeBtn: 1,
                    content: ['${pageContext.request.contextPath}/house/getOneHouseById/' + data.houseId, 'yes'], //iframe的url，yes是否有滚动条
                    end: function () {
                        location.reload();
                    }
                })
     ...
     * */
        function doReSreach(houseNum, deptNum, houseType, address) {
            //执行重载
            table.reload('demo', {
                url: '${pageContext.request.contextPath}/house/getHouseInfo',
                page: {
                    curr: 1 //重新从第 1 页开始
                }
                , where: {
        ...
        $(".del-btn").click(function () {
            ...
            if (checkStatus.data.length < 1) {
                ...
                for (let i = 0; i < checkStatus.data.length; i++) {
                    ids.push(checkStatus.data[i].houseId)
                }
                layer.confirm('确认要删除吗？' + ids.toString(), function (index) {
                    //获取当前点击对象的用户id
                    $.ajax({
                        // 编写json格式，设置属性和值
                        url: "${pageContext.request.contextPath}/house/delHousesByHouseIds",
                        data: {"houseIds": ids.toString()},
                        dataType: "json",
                        type: "post",
                        success: function (data) {
                            if (data.flag == "success") {
        ...
        $("#addHouse").on("click", function () {
            ...
            parent.layer.open({
                ...
                maxmin: true,
                closeBtn: 1,
                content: ['${pageContext.request.contextPath}/pages/house-add.jsp'], //iframe的url，yes是否有滚动条
                end: function () {
                    location.reload();
                }
            })
...
<script type="text/html" id="toolbarDemo">
    <div class="layui-card-header" style="margin: 0;padding: 0">
        ...
        <button class="layui-btn" lay-even="addHouse" id="addHouse"><i
